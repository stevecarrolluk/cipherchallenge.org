...
for column in popularity:
    ...
    b=0
    next=0
    while a!=7 and next<12:
        if next==0:
            e=ord(column[0][1])-ord('a')
            t=ord(column[1][1])-ord('a')
        if next==1:
            e=ord(column[0][1])-ord('a')
            t=ord(column[2][1])-ord('a')
        if next==2:
            e=ord(column[0][1])-ord('a')
            t=ord(column[3][1])-ord('a')
        if next==3:
            e=ord(column[1][1])-ord('a')
            t=ord(column[0][1])-ord('a')
        if next==4:
            e=ord(column[1][1])-ord('a')
            t=ord(column[2][1])-ord('a')
        if next==5:
...
coef={1:1,3:9,5:21,7:15,9:3,11:19,15:7,17:23,19:11,21:5,23:17,25:25}
print(abies)
#figured out it's all 7s
#got rid of weird numbers and counted rest on wolfram alpha
abies=[[7,0],
[7,6],
[7,17],
[7,8],
...
for letter in string:
    if ord('a') <= ord(letter) <= ord('z'):
        result += chr((((ord(letter) - ord('a'))-abies[index%15][1])*coef[abies[index%15][0]]) % 26 + ord('a'))
        index += 1
    elif ord('A') <= ord(letter) <= ord('Z'):
        result += chr((((ord(letter) - ord('A'))-abies[index%15][1])*coef[abies[index%15][0]]) % 26 + ord('A'))
        index += 1
